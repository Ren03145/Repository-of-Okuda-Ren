const canvas = document.getElementById("gameCanvas");  // HTMLからID "gameCanvas" を持つCanvas要素を取得
const ctx = canvas.getContext("2d"); // Canvas要素から2D描画コンテキストを取得します。これを使って図形や画像をCanvasに描画

// プレイヤー画像の読み込みとアニメーション設定
const playerimg = new Image(); // プレイヤー画像オブジェクトを作成
const playerimages = ["Image/player.png", "Image/player2.png"]; // プレイヤーのアニメーション用画像パスを配列で定義
playerimg.src = playerimages[0]; // 最初のプレイヤー画像を初期画像として設定
let animationTimer = 0; // プレイヤーのアニメーション間隔を制御するタイマー
let playerImageIndex = 0; // 現在表示されているプレイヤー画像のアニメーションインデックス

// その他の画像の読み込み
const backgorundimg = new Image(); // 背景画像オブジェクトを作成
backgorundimg.src = "Image/background.png"; // 背景画像のパスを設定
const obstacleimg = new Image(); // 障害物画像オブジェクトを作成
obstacleimg.src = "Image/couple.png"; // 障害物（カップル）画像のパスを設定
const platformimg = new Image(); // 地面の画像
platformimg.src = "Image/platform.png";

// トンビ（敵）画像の読み込みとアニメーション設定
const enemyimgLeft = new Image(); // 左向きのトンビ画像オブジェクトを作成
const enemyimagesLeft = ["Image/enemy.png", "Image/enemy2.png"]; // 左向きトンビのアニメーション用画像パスを配列で定義
enemyimgLeft.src = "Image/enemy.png"; // 最初の左向きトンビ画像を初期画像として設定

const enemyimgRight = new Image(); // 右向きのトンビ画像オブジェクトを作成します。
const enemyimagesRight = ["Image/enemy_right.png", "Image/enemy_right2.png"]; // 右向きトンビのアニメーション用画像パスを配列で定義
enemyimgRight.src = "Image/enemy_right.png"; // 最初の右向きトンビ画像を初期画像として設定

// すべての敵画像を一度に読み込み、配列に格納
// これにより、アニメーション時に画像を再読み込みすることなく効率的に切り替えられる
...
let enemyanimationTimer = 0; // 敵のアニメーション間隔を制御するタイマー
let enemyImageIndex = 0; // 現在表示されている敵画像のアニメーションインデックス
...
hinaImage.src = "Image/ひな.png"; // ヒナ画像のパスを正確に指定する

// BGMの設定（ループ再生と音量）
const bgm = new Audio("Sounds/のどかなみどり.mp3"); // BGMのオーディオオブジェクトを作成
bgm.loop = true; // BGMをループ再生に設定
bgm.volume = 0.5; // BGMの音量を0.5（半分の音量）に設定

// 効果音再生用の関数定義
function onJump() {
    const audio = jumpSoundPool[currentJumpSoundIndex];
    audio.currentTime = 0; // 効果音の再生位置を先頭に戻す
    audio.play(); // ジャンプ効果音を再生
...
function onStomp() {
    ...
    audio.currentTime = 0;
    audio.play(); // 踏みつけ/ダメージ効果音を再生
    currentStompSoundIndex = (currentStompSoundIndex + 1) % stompSoundPool.length; // 次のインデックスへ
}

function onGoal() {
  seGoal.currentTime = 0;
  seGoal.play(); // ゴール効果音を再生
}

function onDie() {
  seDie.currentTime = 0;
  seDie.play(); // ゲームオーバー効果音を再生
}

function onGet() {
  seGet.currentTime = 0;
  seGet.play(); // コイン獲得効果音を再生
}

function drawBackground() {
  ctx.drawImage(backgorundimg, 0, 0, 1000, 600); // 背景画像全体をCanvasに描画
  ctx.fillStyle = "#FFFFFF"; // 雲の色を白に設定
  // 雲の位置とサイズを定義します。
  const clouds = [{ x: 150, y: 50 }, { x: 500, y: 80 }, { x: 850, y: 60 }, { x: 1200, y: 70 }, { x: 1500, y: 50 }];
  for (let cloud of clouds) {
    // カメラの動きの30%の速さで雲をスクロールさせ、遠近感を表現
    let cloudX = cloud.x - (gameState.camera.x * 0.3);
    ctx.beginPath(); // 新しいパスを開始
    ctx.arc(cloudX, cloud.y, 15, 0, Math.PI * 2); // 小さな円を描く
    ctx.arc(cloudX + 15, cloud.y, 20, 0, Math.PI * 2); // 少し大きな円を描く
    ctx.arc(cloudX + 35, cloud.y, 15, 0, Math.PI * 2); // 別の小さな円を描く
    ctx.arc(cloudX + 25, cloud.y - 10, 12, 0, Math.PI * 2); // 上部に小さな円を描き、雲の形を作る
...
document.addEventListener("keydown", (e) => {
  ...
  if ((e.key === "r" || e.key === "R") && (gameState.gameOver || gameState.gameWon)) {
    restartGame(); // ゲームをリスタートする関数を呼び出す
    return; // 処理を終了する
  }
...
function startGame() {
  document.getElementById("hud").style.display = "block"; // HUD（情報表示）を表示
  gameState.gameStarted = true; // ゲーム開始フラグをtrueにする
  restartGame(); // ゲームをリスタートする関数を呼び出し、初期化
}

function restartGame() {
  // ゲームの状態を初期値にリセット、startTimeは現在時刻に設定
  gameState = {
  ...
  if (bgm.paused) { // BGMが停止している場合のみ再生
...
enemies = [
  ...
  goal.x = 3900; // ゴールのX座標を設定
  goal.y = 250; // ゴールのY座標を設定

  particles = []; // パーティクルをクリア

  const gameOverScreen = document.getElementById("gameOver"); // ゲームオーバー画面の要素を取得
  gameOverScreen.style.display = "none"; // ゲームオーバー画面を非表示にする
  gameOverScreen.innerHTML = ""; // ゲームオーバー画面の内容をクリア
  document.getElementById("hud").style.display = "block"; // HUDを再表示
  updateUI(); // UIを更新
}

function updatePlayer() {
  if (player.invulnerable > 0) player.invulnerable--; // 無敵時間を減らす
  // 左右キー入力に応じたプレイヤーの水平方向の速度を設定する
  if (keys["ArrowLeft"]) player.velX = -player.speed;
  else if (keys["ArrowRight"]) player.velX = player.speed;
  else player.velX *= 0.8; // キーが押されていない場合、徐々に減速させる

  // スペースキーが押され、かつプレイヤーが地面にいる場合、ジャンプする
  if (keys[" "] && player.onGround) {
    player.velY = -player.jumpPower; // 上向きの速度を設定
    player.onGround = false; // 地面から離れた状態にする
    onJump(); // ジャンプ効果音を再生
  }

  // 重力の影響を適用します。落下中は加速が大きくなる
  player.velY += (player.velY >= 0) ? 0.08 : 0.5;
  player.x += player.velX; // プレイヤーのX座標を更新
  player.y += player.velY; // プレイヤーのY座標を更新
  player.onGround = false; // デフォルトで地面にいない状態に設定し、衝突判定で更新
  ...
  for (let p of platforms) {
    ...
    if (player.x < p.x + p.width && player.x + player.width > p.x && player.y < p.y + p.height && player.y + player.height > p.y) {
      if (player.velY > 0 && player.y < p.y) {
        ...
        player.y = p.y - player.height; // プレイヤーをプラットフォームの表面に合わせる
        player.velY = 0; // Y方向の速度を0にする
        player.onGround = true; // 地面にいる状態にする
      } else if (player.velY < 0 && player.y > p.y) {
        // 上向きに移動していてプラットフォームの下面に衝突した場合
        player.y = p.y + p.height; // プレイヤーをプラットフォームの下面に合わせる
        player.velY = 0; // Y方向の速度を0にする
      } else if (player.velX > 0) {
        // 右向きに移動していてプラットフォームの左側に衝突した場合
        player.x = p.x - player.width; // プレイヤーをプラットフォームの左側に合わせる
        player.velX = 0; // X方向の速度を0にする
      } else if (player.velX < 0) {
        // 左向きに移動していてプラットフォームの右側に衝突した場合
        player.x = p.x + p.width; // プレイヤーをプラットフォームの右側に合わせる
        player.velX = 0; // X方向の速度を0にする
      }
    }
  }
  ...
  if (gameState.camera.x < 0) gameState.camera.x = 0; // カメラがCanvasの左端を超えないように制限する
}
...
function getCollisionCircle(obj) {
  ...
  return { cx, cy, radius }; // 中心座標と半径を返す
}
...
function updateCoins() {
  for (let c of coins) {
    if (!c.collected && checkCollision(player, c)) { // コインが未収集で、プレイヤーと衝突した場合
      c.collected = true; // コインを収集済みに設定する
      gameState.coins++; // コイン数をカウントアップする
      onGet(); // コイン獲得効果音を再生
    }
  }
}
...
function checkGoal() {
  ...
  if (player.x < goal.x + goal.width && player.x + player.width > goal.x && player.y < goal.y + goal.height && player.y + player.height > goal.y) {
    if (gameState.gameWon) return; // すでにゲームクリアしていれば何もしない
    gameState.gameWon = true; // ゲームクリアフラグをtrueに設定
    bgm.pause(); // BGMを停止
    onGoal(); // ゴール効果音を再生

    createConfetti(goal.x + goal.width / 2, goal.y); // ゴール位置で紙吹雪を生成する

    document.getElementById("hud").style.display = "none"; // HUDを非表示にする
    // スコア計算
    const timeBonus = Math.max(0, 3000 - gameState.elapsedTime * 70); // 時間ボーナス（短いほど高得点）
    const coinBonus = gameState.coins * 200; // コインボーナス
    const enemyBonus = gameState.stompedEnemies * 100; // 敵撃破ボーナス
    gameState.score = timeBonus + coinBonus + enemyBonus; // 合計スコアを計算する
...
function playerDie() {
  if (player.invulnerable > 0) return; // 無敵時間中であれば死亡処理をスキップ
  gameState.lives--; // ライフを1減らす
  player.invulnerable = 120; // 120フレーム（約2秒）の無敵時間を設定
  if (gameState.lives <= 0) { // ライフが0以下になった場合
    gameState.gameOver = true; // ゲームオーバーフラグをtrueに設定
    bgm.pause(); // BGMを停止
    onDie(); // ゲームオーバー効果音を再生
    document.getElementById("hud").style.display = "none"; // HUDを非表示にする
    const gameOverScreen = document.getElementById("gameOver"); // ゲームオーバー画面の要素を取得
    const failColor = "#a94442"; // 失敗時の色を設定
...
function drawPlayer() {
  ...
  // 無敵時間中、プレイヤーを点滅させて描画する
  if (player.invulnerable === 0 || Math.floor(player.invulnerable / 5) % 2 === 0) {
    ctx.globalAlpha = 1; // 透明度を元に戻す
    ctx.drawImage(playerimg, player.x - gameState.camera.x, player.y, player.width, player.height); // プレイヤー画像を描画する
  }
  ctx.restore(); // 保存した描画状態を復元する
}
...
function drawObstacles() {
  for (let o of obstacles) { ctx.drawImage(obstacleimg, o.x - gameState.camera.x, o.y, o.width, o.height); } // 障害物画像を描画
}
...
function drawGoal() {
    ...
    const toriiHeight = 150; // 鳥居全体の高さ

    // 土台を長くするための新しい変数
    const originalGateWidth = 100; // 元の鳥居の横幅
    const extendedBaseWidth = 150; // 拡張する土台の新しい幅
    const baseHeight = 30; // 土台の高さ
    const baseOffsetFromGround = 0; // 地面からのオフセット

    const hinaWidth = 50; // ヒナの表示幅を大きく
    const hinaHeight = 50; // ヒナの表示高さを大きく
    const hinaSpacing = 5; // ヒナ同士の間隔を詰める
    ...
    // ヒナグループの開始X座標を計算
    // 鳥居の右端 (gateX + originalGateWidth) から少しオフセット
...
function updateAndDrawParticles() {
  for (let i = particles.length - 1; i >= 0; i--) { // 後ろからループすることで、要素を削除してもインデックスがずれないようにする
    const p = particles[i];
    p.life--; // ライフを減らす
    if (p.life <= 0) { // ライフが尽きたら
      particles.splice(i, 1); // パーティクルを配列から削除
      continue; // 次のパーティクルへ。
    }
    p.velY += p.gravity; // 重力を適用
    p.x += p.velX; // X座標を更新
    p.y += p.velY; // Y座標を更新

    ctx.fillStyle = p.color; // パーティクルの色を設定
    ctx.fillRect(p.x - gameState.camera.x, p.y, p.size, p.size); // パーティクルを描画
  }
}

function updateUI() {
  if (!gameState.gameStarted) return; // ゲームが開始されていなければ更新しない
...
function gameLoop() {
  ...
  if (gameState.gameStarted) { // ゲームが開始されている場合のみ以下の処理を実行
    if (!gameState.gameOver && !gameState.gameWon) { // ゲームオーバーでもゲームクリアでもない場合
      gameState.elapsedTime = Math.floor((Date.now() - gameState.startTime) / 1000); // 経過時間を計算
      updatePlayer(); // プレイヤーの状態を更新
      updateObstacles(); // 障害物の状態を更新
      updateEnemies(); // 敵の状態を更新
      updateCoins(); // コインの状態を更新
      checkGoal(); // ゴール判定を行う
    }
    drawPlatforms(); // プラットフォームを描画
    drawCoins(); // コインを描画
    drawObstacles(); // 障害物を描画
    drawEnemies(); // 敵を描画

    // プレイヤーが十分に動いている場合のみアニメーションを実行
    // 速度の絶対値が0.05より大きい場合
    if (Math.abs(player.velX) > 0.05) { 
      ...
      // タイマーをリセット 
      animationTimer = 0;
      // 次の画像インデックスに切り替え(0→1→0→...)
      playerImageIndex = (playerImageIndex + 1) % playerimages.length;
      // プレイヤー画像ソースを更新
      playerimg.src = playerimages[playerImageIndex];
      }
      // プレイヤーがほぼ停止している場合
    } else { 
      // 最初の画像に戻す
    ...
    updateAndDrawParticles(); // パーティクルを更新し描画

    updateUI(); // UIを更新
  }
  requestAnimationFrame(gameLoop); // 次のフレームで`gameLoop`関数を再度呼び出します。これによりスムーズなアニメーションが実現する
}
